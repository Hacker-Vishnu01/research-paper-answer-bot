"""
Benchmark BM25 retrieval on the 20-question evaluation dataset.

Results are evaluated at the paper level so they can be compared
directly with the dense MPNet retrieval benchmark.
"""
...
OUTPUT_PATH = PROJECT_ROOT / "evaluation" / "bm25_retrieval_results.csv"
...
def main():
    ...
    # ---------------------------------------------------------
    # Create the same chunks used by MPNet
    # ---------------------------------------------------------
    print("\nCreating chunks...")
    ...
    retriever = BM25RetrieverWrapper(chunks)

    index_time = time.perf_counter() - index_start

    print(f"BM25 index built in {index_time:.4f}s")
    ...
    for item in questions:
        ...
        # Convert retrieved chunks to paper IDs.
        retrieved_paper_ids = [
            chunk.metadata.paper_id
            for chunk, score in retrieved
        ]

        # Deduplicate paper IDs while preserving ranking order.
        ...
        print(
            f"{question_id}: "
            f"Top-1={unique_retrieved_paper_ids[0] if unique_retrieved_paper_ids else 'N/A'} "
            f"| MRR={mrr:.4f} "
    ...
    print(f"Index build:    {index_time:.4f}s")
    ...
    print("\nBM25 retrieval benchmark completed successfully.")
